# calculate the NMI value from the clustering results of DySC for all cufoffs. By Ze-Gang Wei.
...
usage: python NMI_dysc_all_cutoff.py --input <otu directory>  -r <reference NMI label file> -s <sequence file>

Example: python NMI_dysc_all_cutoff.py -i ~/dysc/ -r tax_assignments_NMI_label.txt -s sequence.fa
----------------------- All file path needs absolute path.
--input/-i      (required) The OTU clustered output file directory by DySC, need absolute path.
