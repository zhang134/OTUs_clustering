# calculate the NMI value from the clustering results of VSEARCH for all cufoffs. By Ze-Gang Wei.
...
usage: python NMI_vsearch_all_cutoff.py --input <otu directory>  -r <reference NMI label file> -s <sequence file>

Example: python NMI_vsearch_all_cutoff.py -i ~/usearch/ -r tax_assignments_NMI_label.txt -s sequence.fa
----------------------- All file path needs absolute path.
--input/-i      (required) The OTU clustered output file directory by VSEARCH, need absolute path.
...
"""
	...
	usearch = {}
	cutoffss = []
	cutoff_all = ['vsearch99','vsearch98','vsearch97','vsearch96','vsearch95','vsearch94','vsearch93','vsearch92','vsearch91','vsearch90',\
		      'vsearch89','vsearch88','vsearch87','vsearch86','vsearch85']
	cutoff_files = {}
			...
			if cc in ff:
				...
				while row:
					if row.startswith('S'):
						...
						line_split = row.split()
						header = line_split[8]
						cluster[header] = cluster[line_split[9]]
						row = oo.readline().strip()
					else:
						row = oo.readline().strip()
